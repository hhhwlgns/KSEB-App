...
import { createWarehouseRequest, createRackWarehouseRequest, getProducts, getClients } from '../lib/api';
import { useWarehouseForm } from '../context/WarehouseFormContext';
import { Product, Client, RackItem, Rack } from '../types';
...
export default function WarehouseFormScreen() {
  ...
  const isRackMode = useMemo(() => !!route.params?.rack, [route.params]);
  const rack = useMemo(() => route.params?.rack, [route.params]);
  ...
  const { data: products, isLoading: isLoadingProducts } = useQuery({ queryKey: ['products'], queryFn: getProducts, enabled: !isRackMode });
  const { data: clients, isLoading: isLoadingClients } = useQuery({ queryKey: ['clients'], queryFn: getClients });

  const singleItemMutation = useMutation({
    ...
    onError: (error) => toast.error(error.message || '요청 등록 중 오류가 발생했습니다.'),
  });

  const rackMutation = useMutation({
    mutationFn: (vars: { rack: Rack, type: 'inbound' | 'outbound', notes: string, client: Client }) => 
      createRackWarehouseRequest(vars.rack, vars.type, vars.notes, vars.client),
    onSuccess: () => {
      toast.success('입출고 요청이 성공적으로 등록되었습니다.');
      queryClient.invalidateQueries({ queryKey: ['warehouseRequests'] });
      navigation.goBack();
    },
    onError: (error) => toast.error(error.message || '랙 요청 등록 중 오류가 발생했습니다.'),
  });

  const isSaving = singleItemMutation.isPending || rackMutation.isPending;

  const validateForm = (): boolean => {
    const newErrors: { [key: string]: string } = {};
    if (!isRackMode && !selectedProduct) newErrors.product = '품목을 선택해주세요.';
    if (!selectedClient) newErrors.client = '거래처를 선택해주세요.';
    if (!isRackMode && (!formData.quantity.trim() || isNaN(Number(formData.quantity)) || Number(formData.quantity) <= 0)) {
      newErrors.quantity = '올바른 수량을 입력해주세요.';
    }
    if (timeDigits.length !== 4) newErrors.time = '올바른 시간 형식(HH:MM)을 입력해주세요.';
    setErrors(newErrors);
  ...
  const handleSubmit = () => {
    ...
    if (isRackMode && rack) {
      rackMutation.mutate({ rack, type: formData.type, notes: formData.notes, client: selectedClient! });
    } else {
      ...
      singleItemMutation.mutate(requestData);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Header title={isRackMode ? "랙 단위 요청" : "입출고 요청 등록"} showBack />
          ...
          <View style={styles.form}>
            <View style={styles.inputContainer}>
              ...
              <View style={styles.typeSelector}>
                <TouchableOpacity style={[styles.typeButton, formData.type === 'inbound' && styles.typeButtonActive]} onPress={() => setFormData(prev => ({ ...prev, type: 'inbound' }))}>
                  <Text style={[styles.typeButtonText, formData.type === 'inbound' && styles.typeButtonTextActive]}>입고</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.typeButton, formData.type === 'outbound' && styles.typeButtonActive]} onPress={() => setFormData(prev => ({ ...prev, type: 'outbound' }))}>
                  <Text style={[styles.typeButtonText, formData.type === 'outbound' && styles.typeButtonTextActive]}>출고</Text>
                </TouchableOpacity>
              </View>
            </View>

            {isRackMode ? renderRackContents() : (
              <>
